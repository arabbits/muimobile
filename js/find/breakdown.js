...
$(function(){
    var page = 1;
    var user_name ,cart_licens,obd_id;
    console.log(typeof(typeof{}));
    loadPage(page)
    function loadPage(page,user_name,cart_licens,obd_id){
        $.ajax({
            url:api+"smobileListTrouble",
            type:'post',
    ...
    $('.break_twins ').on('tap','span',function(){
        ...
        // .parent().siblings().find('span').removeClass('show_dots')
    })
    // 处理
    $('.break_twins ').on('tap','.overThis',function(){
        var trouble_id = $(this).data('id')
        console.log(trouble_id);
        mui.confirm('确定将该故障设为已处理吗?','提示',['未处理','已处理'],function(index){
            if(index.index==1){
                console.log(trouble_id);
                $.ajax({
                    url:api+"smobileDelTrouble",
                    type:'post',
                    headers:{'sellermobileauthkey':token},
                    beforeSend:function(){
                        $('.loading').show()
